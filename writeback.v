module writeback_unit(
    ...
    always @(*) begin
        ...
        if (opcode == 6'b000000) begin          // MOV Immediate
            ...
            rf_addr  = instruction[25:21];      // Rdst
            data_out = alu_result;              // Passed through ALU
            rf_we    = 1;
        end 
        else if (opcode == 6'b000010) begin     // LOAD
            rf_addr  = instruction[25:21];      // Rdst
            data_out = mem_read_data;           // Data from Memory
            rf_we    = 1;                       // Corrected: Write to RF
        end 
        else if (opcode == 6'b000011) begin     // STORE
            mem_addr = instruction[25:18];       // Dst Address
            data_out = alu_result;              // Data from Rsrc (via ALU)
            mem_we   = 1;
        end 
            ...
            rf_addr  = instruction[25:21];      // Rdst1 (Lower byte)
            data_out = alu_result;
            rf_we    = 1;
        end 
